...
app.get("/pharmach_list", (req, res) => {
    let api = async() => {  // 교차된 리소스 보안문제를 해결하기 위함
        ...
        try {
            response = await axios.get("http://apis.data.go.kr/B552657/ErmctInsttInfoInqireService/getParmacyListInfoInqire", {
                params: {
                    "serviceKey": "erw1qPjO2qB/Tm5fpAvLoOLSZHXEAHBU2AXETayn/wThw0/81G4TWuI/LjN+9ln3CA4R5jpM4Gjc2IGWgBQc1g==",
                    "Q0": req.query.Q0,                 // 주소(시도)
                    "Q1": req.query.Q1,                 // 주소(시군구)
                    "QT": req.query.QT,                 // 월~일요일, 공휴일: 1~8
                    "QN": req.query.QN,                 // 기관명
                    "ORD": req.query.ORD,               // 순서
                    "pageNo": req.query.pageNo,         // 페이지번호
                    "numOfRows": req.query.numOfRows    // 목록건수
                }
            });
    ...
    .then((response) => {
      ...
      res.json({ response: response.data.response.body});
    })
    .catch((error) => {
